...
extension Link {
    public init(
        _ destination: @escaping () -> DestinationType,
        _ build: ((DestinationRequest<DestinationType>) -> Request)? = nil,
    ...
    @inlinable public init<T>(
        ...
        _ build: ((DestinationRequest<DestinationType>) -> Request)? = nil,
        @ViewBuilder label: @escaping () -> Label
    ) where DestinationType == ViewDestination<T> {
    ...
    @inlinable public init<T>(
        _ viewController: @escaping () -> T,
        _ build: ((DestinationRequest<DestinationType>) -> Request)? = nil,
        @ViewBuilder label: @escaping () -> Label
    ) where DestinationType == ViewDestination<ViewControllerWrapper<T>> {
    ...
    @inlinable public init<T>(
        ...
        _ build: ((DestinationRequest<DestinationType>) -> Request)? = nil,
        @ViewBuilder label: @escaping () -> Label
    ) where DestinationType == ViewDestination<ViewWrapper<T>> {
