...
extension Routing {
    class RegularExpression: NSRegularExpression {
        ...
        func matchResult(for string: String, parameters: inout [String: Any]?) -> Bool {
            var extractedParameters: [String: Any] = [:]

            let string = stripURLQueryItems(from: string, parameters: &extractedParameters)

            let allMatches = matches(in: string, options: [], range: NSRange(location: 0, length: string.count))
            ...
            for result in allMatches {
                for identifier in (1..<result.numberOfRanges) {
                    if identifier > groupNames.count {
                        break
                    }

                    let parameterName = groupNames[identifier - 1]
                    let parameterValue = (string as NSString).substring(with: result.range(at: identifier))
                    extractedParameters[parameterName] = parameterValue
                }
            }
